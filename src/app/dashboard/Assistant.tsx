...
function answer(q0: string, c: Ctx): Ans {
  ...
  /* الالتزام */
  if (has(q, "التزام", "الالتزام", "متأخر عن المدة", "SLA"))
    return {
  ...
  /* التقارير الربعية */
  if (has(q, "ربع", "الربعية", "جلسات", "جلسة"))
    return {
      title: `التقارير الربعية — الربع ${curQ}`,
      icon: "calendar-check",
      lines: ents.length
        ? qLate.length
          ? qLate.map((r) => `لم تُعلَّم بعد: ${txt(r.data?.name)}`)
          : ["كل جهاتك مُعلَّمة لهذا الربع 👌"]
  ...
  /* الجهات */
  if (has(q, "جهات", "جهاتي", "استراتيجيات", "الاستراتيجيات"))
    return {
